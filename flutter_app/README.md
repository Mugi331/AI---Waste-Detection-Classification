...
1. Uncomment the `ApiDetectionService` class near the bottom of the
   file and add the two imports it needs (`dart:convert` and
   `package:http/http.dart as http`).
2. Change the last line of the file from:
   ```dart
   ...
   ```
3. Nothing else in the app needs to change — screens and widgets only
   depend on the `DetectionService` interface and `DetectionResponse`
   model, not on how the data was obtained.

Expected backend JSON shape (already matched by
`DetectionResponse.fromJson`):

```json
{
  "detections": [
    { "class": "Plastic", "confidence": 0.91, "bbox": [120, 80, 310, 420] },
    { "class": "Metal", "confidence": 0.84, "bbox": [410, 150, 530, 350] }
  ],
  "image_width": 1000,
  "image_height": 1000
}
...
(`image_width` / `image_height` are the pixel dimensions the `bbox`
values are relative to — add these on the backend if not already
present, since they're what let the Flutter overlay scale boxes
correctly regardless of how the image is displayed.)

## 7. Notes on model-agnosticism

- No file references YOLO, `.pt` weights, or any detector-specific
  concept. `DetectionResult` only ever sees `{ class, confidence,
  bbox }`, so SSD, Faster R-CNN, or EfficientDet can replace YOLOv8n
  on the Python side with zero Flutter changes, as long as the API
  response is normalised to that shape.
